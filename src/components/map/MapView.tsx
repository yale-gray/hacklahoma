...
export function MapView() {
  ...
  const handleNodeRightClick = useCallback(
    (node: GraphNode) => {
      ...
      if (!node.clusterColor) return;

      // Find the tag for this cluster
      const tagToNotes = new Map<string, string[]>();
      for (const note of notes) {
        const allTags = [...new Set([...note.tags, ...(note.autoTags ?? [])])];
        for (const tag of allTags) {
      ...
      // Find the largest tag group this node belongs to
      let bestTag: string | null = null;
      let bestCount = 0;
      for (const [tag, ids] of tagToNotes.entries()) {
        if (ids.includes(node.id) && ids.length >= mapColorThreshold && ids.length > bestCount) {
          bestTag = tag;
      ...
      if (bestTag) {
        const chapter: Chapter = {
          tag: bestTag,
          count: bestCount,
          noteIds: tagToNotes.get(bestTag)!,
        };
        setSelectedChapter(chapter);
      }
    },
    [notes, mapColorThreshold]
  );
    ...
    >
      ...
      <div className="absolute top-4 left-4 bg-[#1a0f0a]/90 rounded-lg p-3 shadow-lg backdrop-blur-sm border border-[#d4a574]/30">
        ...
        <p className="text-[9px] italic mt-2 text-[#8b7355] font-serif">
          Click a node to open • Right-click cluster to synthesize
        </p>
      </div>
